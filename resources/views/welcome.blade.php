...
<form action="{{ route('register') }}" method="POST" class="w-1/3 mx-auto mt-10 border rounded p-3">
    ...
    <div class="mb-3 flex">
        <div class="w-1/2 mx-3">
            ...
            <input type="text" class="form-control" name="name" id="name" placeholder="Hogy hívnak?" value="{{ old('name') }}" required>
            @error('name')
        ...
        <div class="w-1/2 mx-3">
            ...
            <input type="email" class="form-control" name="email" id="email" placeholder="Email címed" value="{{ old('email') }}" required>
            @error('email')
            <p class="text-red-500 text-xs mt-1">{{$message}}</p>
            @enderror
        </div>
    ...
    <div class="mb-3 flex">
        ...
        <div class="w-1/2 mx-3">
            ...
            <input list="cities" class="form-control" id="city" name="city" placeholder="Kezdj el írni!" value="{{ old('city') }}" required>
            <datalist id="cities">
    ...
    <div class="mb-3 flex">
        <div class="w-1/2 mx-3">
            ...
            <input type="tel" class="form-control" name="telefonszam" id="telefonszam" placeholder="Csak számokat adj meg!" pattern="\d+" value="{{ old('telefonszam') }}" required>
            @error('telefonszam')
            <p class="text-red-500 text-xs mt-1">{{$message}}</p>
            @enderror
        </div>
    ...
    <div class="mb-3 flex">
        <div class="w-1/2 mx-3">
            ...
            <input type="password" class="form-control" name="password" id="password" placeholder="Jelszavad" required>
            @error('password')
            <p class="text-red-500 text-xs mt-1">{{$message}}</p>
            @enderror
        </div>
        <div class="w-1/2 mx-3">
            ...
            <input type="password" class="form-control" name="password_confirmation" id="password-confirmation" placeholder="Jelszavad ismét" required>
    ...
    <div class="mx-3">
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="gender" id="radio1" value="male" {{ old('gender') == 'male' ? 'checked' : '' }} required>
            <label class="form-check-label" for="radio1">
                Férfi
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="gender" id="radio2" value="female" {{ old('gender') == 'female' ? 'checked' : '' }} required>
            <label class="form-check-label" for="radio2">
                Nő
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="gender" id="radio3" value="other" {{ old('gender') == 'other' ? 'checked' : '' }} required>
    ...
    <div class="form-check mx-3 mt-6">
        <input class="form-check-input" type="checkbox" name="felhasznalasi" id="felhasznalasi" value="1" {{ old('felhasznalasi') == '1' ? 'checked' : '' }} required>
